...
from render.models import ReportParameter, ParameterType, ReportMetadata
...
class TestEnumQuery:
    ...
    async def test_execute_enum_query(self, temp_reports_dir):
        ...
        enum_query_file.write_text("SELECT unnest(ARRAY['val1', 'val2', 'val3']) AS value;")
    ...
    async def test_metadata_with_enum_query(self, temp_reports_dir):
        ...
        with patch('render.services.render_service.repository') as mock_repo, \
             patch('render.services.render_service.query_executor') as mock_executor:
