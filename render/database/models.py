...
class Render(Base):
    ...
    parameter_hash = Column(String(64), primary_key=True, comment="SHA256 hash of report_id + parameters")
    
    report_id = Column(String(255), nullable=False, index=True, comment="Report identifier")
    parameters_json = Column(Text, nullable=False, comment="JSON-encoded parameters")
    
    status = Column(String(50), nullable=False, index=True, comment="PENDING, RUNNING, COMPLETED, FAILED")
    ...
    error_message = Column(Text, nullable=True, comment="Error details if status is FAILED")
    
    created_at = Column(DateTime, nullable=False, default=datetime.utcnow, comment="When render was requested")
    started_at = Column(DateTime, nullable=True, comment="When rendering started")
    completed_at = Column(DateTime, nullable=True, index=True, comment="When rendering finished")
